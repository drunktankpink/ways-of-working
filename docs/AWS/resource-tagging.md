...
- Do not add personally identifiable information (PPI) or other confidential or sensistive information in tags. Tags are not intended to be used for provate or sensitive data
...
- Remember that it is easy to modify tags to accomodate changing business requirements, however, condsider the ramifications of future changes, especially in relation to tag-based access control, automation or upstream billing reports
...
| --------- | ---------- | -------- | -------- |
| <ui><li>**Name -** Identify resources with a unique name</li><li>**Application ID -** Identify resources that belong to a specific application</li><li>A**pplication role / service -** Describe the function of of a particular resource (web server, bastion, database, message broker)</li><li>**Cluster -** Indentify resource groups that share a common configuration & perform a specific function for an application</li><li>**Environment -** Distinguist between development, test & production environments</li><li>**Version -** Distinguist between versions of applications or resources</li></ul> | <ui><li>**Date/Time -** Identify the date or time a resources should be started, stopped, deleted or rotated</li><li>**Opt in/Opt out -** Indicate whether a resource should be included in an automated activity such as starting, stopping or resizing instances</li><li>**Security -** Determine requires such as encurption or enabling of Amazon VPC flow logs; identify resources that need extra security (security grops or route tables)</li></ul> | <ui><li>**Project -** Identify the project that the resources support</li><li>**Owner -** Identify who is responsible for resource</li><li>**Cost Center/Business Unit -** Identify the cost center or business unit associated with a resource, typically for cost allocation</li><li>**Customer -** Identify a client that resources serve</li></ul> | <ui><li>**Confidentiality -** Identify specific data confidentiality levels for a resource</li><li>**Compliance -** Identify if resources need to adhere to specific compliance requirements</li></ul> |
...
| cost_centre | numberical | "384786" | The cost centre for the resources |
...
| id | numerical | "01" | identifer |
| name | programatic | publiccloud-prod-backup-01 | Depending on your naming convention, this should ideally be handled by your IaC deployment |


service_name - Allow for grouping of resources by service, across environemtns or projects.
...
For any tagging strategy to be effective, you need to apply it consistently and programatically across AWS resources. Two approaches to governing tags are reactive and proactive
